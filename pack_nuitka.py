...
DIST_DIR = Path("dist")
VERSION_FILE = Path("src") / "ipa_clutch_batch" / "version.py"
ENTRY_FILE = Path("src") / "ipa_clutch_batch" / "main.py"
LIBIMOBILE_SOURCE = Path("libimobile")
LIBIMOBILE_TARGET = DIST_DIR / "libimobile"
...
def copy_libimobile_directory() -> None:
    """Copy the libimobile tools next to the generated executable."""
    print("[Post-Build] Copying libimobile tools...")
    if not LIBIMOBILE_SOURCE.is_dir():
        print(f"[Post-Build] Warning: directory not found: {LIBIMOBILE_SOURCE}")
        return
    ...
    print(f"[Post-Build] Copied tools to: {LIBIMOBILE_TARGET}")
