...
def get_app_root() -> Path:
    ...
    - CLI wrapper from editable install
    """
    if is_packaged_app():
    ...
    if hasattr(__main__, "__file__"):
        ...
        if main_file.parent.is_dir():
            if main_file.parent.name == "src":
                return main_file.parent.parent.resolve()
...
def get_resource_root(app_root: Path) -> Path:
    ...
    PyInstaller extracts bundled resources to sys._MEIPASS. In development,
    resources are read from the project root.
    """
